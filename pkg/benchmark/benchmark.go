...
func BenchmarkAPI(baseURL string) error {
	tests := []struct {
		name   string
		...
		{
			name: "Beamforming Algorithm",
			config: BenchmarkConfig{
				URL:    baseURL + "/api/v1/algorithm/beamforming",
				Method: "POST",
				Payload: map[string]interface{}{
					"experiment_id": "bench_test",
					"params": map[string]interface{}{
						"element_count":       64,
						"target_direction":    0.5,
						"interference_angles": []float64{0.2, 0.8},
						"snr_threshold":       0.9,
						"max_iterations":      100,
					},
				},
